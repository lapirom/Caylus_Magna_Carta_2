...
class Player():
    ...
    money_resources = None  # type: Dict[module_Resource.MoneyResource, int]
    ...
    def __init__(self, color_player: ColorPlayer):
        ...
        self.current_money_resources = None  # type: Dict[module_Resource.MoneyResource, int]
    ...
    def txt_name_money_resources_workers_PPs_deck(self, with_money: bool, with_resources: bool, with_workers: bool,
        ...
        if with_money:
            get_name_money_resources_workers_PPs_deck.append(str(self.current_money_resources[module_Resource.Money.money]) + ' ' +
                                                             module_Resource.Money.money.name + '(s)')
        if with_resources:
            get_name_money_resources_workers_PPs_deck.append('resources = (' + TXT_SEPARATOR.join(
                [str(self.current_money_resources[resource]) + ' ' + resource.name + '(s)'
                 for resource in module_Resource.Resource.resources.values() if not resource.is_wild()] +
                [str(self.current_money_resources[module_Resource.Resource.get_wild_resource()]) + ' ' +
                module_Resource.Resource.get_wild_resource().name + '(s)']) + ')')
        if with_workers:
    ...
    def resource_all_payments(self, resource_costs):  # -> List[Dict[module_Resource.Resource, int]]
        ...
        resource_all_payments = list()  # type: List[Dict[module_Resource.Resource, int]]
        if None in resource_costs.keys():
            # We have to consider the case of any cube resources.
            resources_not_wild = [resource for resource in module_Resource.Resource.resources.values()
                                  if not resource.is_wild()]  # type: List[module_Resource.Resource]
            for resources_not_wild_to_use in itertools.combinations_with_replacement(resources_not_wild,
                                                                                     -resource_costs[None]):
                resource_costs_to_use = {}  # type: Dict[module_Resource.Resource]
                for resource in module_Resource.Resource.resources.values():  # All resources, including wild.
                ...
                resource_payments = self.resource_payments(resource_costs_to_use)  # type: Dict[module_Resource.Resource, int]
            ...
            resource_payments = self.resource_payments(resource_costs)  # type: Dict[module_Resource.Resource, int]
    ...
    def resource_payments(self, resource_costs):  # Optional[Dict[module_Resource.Resource, int]]
        """Get one payment of resources for the cost of resources without the case of any cube resources.
        Remark: wild resource is used for missing resources if and only if it is necessary."""
        resource_payments = {}  # type: Dict[module_Resource.Resource, int]
        wild_resource = module_Resource.Resource.get_wild_resource()  # type: module_Resource.Resource
        for resource in module_Resource.Resource.resources.values():  # All resources, including wild.
    ...
    def n_max_batches_to_castle(self, castle_phase: module_Phase.CastlePhase) -> int:
        ...
        resources_not_wild = {resource_not_wild: self.current_money_resources.get(resource_not_wild)
                              ...
                              }  # type: Dict[module_Resource.Resource, int]
        n_wild = self.current_money_resources.get(module_Resource.Resource.get_wild_resource())  # type: int
    ...
    def consume_n_max_batches_to_castle(self, n_max_batches_to_castle: int, castle_phase: module_Phase.CastlePhase) -> int:
        ...
        self.current_money_resources[module_Resource.Resource.get_wild_resource()] -= n_necessary_wild_resources
    ...
    def tot_n_prestige_pts(self, buildings_road) -> int:
        ...
        return self.current_n_prestige_pts + \
               ...
               sum([self.current_money_resources.get(resource_wild)
                    for resource_wild in module_Resource.Resource.resources.values()
    ...
    @abc.abstractmethod
    def choose_buy_resource(self, money_resource_cost: module_Resource.MoneyResource, qty_cost: int, resource_gain_choices,
                            qty_gain: int) -> module_Resource.Resource:
    ...
    def choose_exchange_resource(self, can_no_use_effect: bool, qty_cost: int, resource_cost_choices,
                                 money_resource_gain: module_Resource.MoneyResource, qty_gain: int) -> module_Resource.Resource:
...
class HumanPlayer(Player):
    ...
    def choose_buy_resource(self, money_resource_cost: module_Resource.MoneyResource, qty_cost: int, resource_gain_choices,
                            qty_gain: int) -> module_Resource.Resource:
        possibilities = list(module_Building.ABBREV_NO_USE_EFFECT)  # type: List[str[1]]
        abbrev_resource_name_resource = {}  # type: Dict[str[1],module_Resource.Resource] # E.g. {'F': food, ...}.
    ...
    def choose_exchange_resource(self, can_no_use_effect: bool, qty_cost: int, resource_cost_choices,
                                 money_resource_gain: module_Resource.MoneyResource, qty_gain: int) -> module_Resource.Resource:
        abbrev_resource_name_resources =module_Resource.Resource.get_name_abbreviation_resources(resource_cost_choices)
        possibilities = list()  # type: List[str[1]]
        if can_no_use_effect:
            possibilities.append(module_Building.ABBREV_NO_USE_EFFECT)
